...
async def sniff(message: Message):
	...
	if "npv2" in replied.document.file_name:
		...
		chat = "@hat_decryptor_bot"
		await message.edit("```Wait for result... ```")
		msgs = []
		ERROR_MSG = "Unblok bot @hat_decryptor_bot untuk menggunakan command ini"
		try:
			async with userge.conversation(chat) as conv:
				try:
					await conv.forward_message(replied)
				...
				if "Satisfied with the result" in result:
					result = result.replace("please enter code  by viste this website","").replace("https://t.me/all_in_crypto_team/45","").replace("for Hc Tls Npv4 and other files join ","")
					result = f"{result}".replace("@all_in_one_decrypit","https://face.nakta2.com")
		except StopConversation:
			pass 
		try:
			await message.edit(f"{result}")
